...
# Description
################################################################################
# The lists of cell type markers are expanded using the PCSF algorithm.
# Output files:
# - New cell type marker files with the extended lists of genes.
################################################################################
...
# Inputs
################################################################################
load('../../results/ucam_sanyal/networks/MASLD_unified_undirected_network_with_semantics.RData')
...
for (f in files) {
  ...
  to_remove <- c(selected_genes[grep('^RPS', selected_genes)],
                 ...
                 selected_genes[grep('^CHEBI', selected_genes)])
