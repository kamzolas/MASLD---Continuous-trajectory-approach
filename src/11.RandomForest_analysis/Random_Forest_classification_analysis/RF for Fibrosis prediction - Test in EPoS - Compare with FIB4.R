...
run_rf_analysis <- function(selected_genes_path, gene_set_name,
   ...
   # ---- Predict on EPoS ----
   pred_epos <- predict(rf_cv, newdata = z_epos)
   prob_epos <- predict(rf_cv, newdata = z_epos, type = "prob")[,"Advanced"]
   conf_epos <- confusionMatrix(pred_epos, z_epos$fibrosis_bin)
   roc_epos <- roc(z_epos$fibrosis_bin, prob_epos)
   auc_epos <- auc(roc_epos)
   
   ci_epos <- ci.auc(roc_epos)
   
   # ---- ROC Comparison with FIB-4 ----
   roc_fib4 <- roc(z_epos$fibrosis_bin, epos_meta$FIB4)
   auc_fib4 <- auc(roc_fib4)
   
   ci_fib4 <- ci.auc(roc_fib4)
   
   cat(gene_set_name, "- EPoS AUC =", auc_epos,
       "CI:", ci_epos[1], "-", ci_epos[3], "\n")
   
   cat("FIB-4 AUC =", auc_fib4,
       "CI:", ci_fib4[1], "-", ci_fib4[3], "\n")
   
   
   
   delong_test <- roc.test(roc_epos, roc_fib4, method = "delong")
   cat("DeLong p-value vs FIB-4:", delong_test$p.value, "\n")
   
   
   roc_epos_smooth <- smooth(roc_epos)
   roc_fib4_smooth <- smooth(roc_fib4)
   
   
   roc_df <- bind_rows(
      data.frame(
         FPR = 1 - roc_epos_smooth$specificities,
         TPR = roc_epos_smooth$sensitivities,
         Method = paste0("RF ", gene_set_name, " (AUC=", round(auc_epos,2), ")")
      ),
      data.frame(
         FPR = 1 - roc_fib4_smooth$specificities,
         TPR = roc_fib4_smooth$sensitivities,
...
gene_sets <- list(
   ...
   "3-gene published panel" = "data/External_3_gene_panel_for_additional_validation.csv"
)
...
roc_all <- roc_all %>%
   filter(Method != "FIB-4 (AUC=0.76)" | !duplicated(FPR))



# === ROC CONFIDENCE BANDS === #

ci_list <- lapply(results_list, function(res){
   
   ci <- ci.se(res$roc_obj, specificities = seq(0,1,length=50))
   ...
   data.frame(
      ...
      Sens_mid = ci[,2],
      Sens_high = ci[,3],
      Method = paste0("RF ", names(res$auc))
   )
...
p_roc_all <- ggplot(roc_all, aes(FPR, TPR, color = Method)) +
   ...
   scale_size_manual(values = c(
      "RF 57-gene (AUC=0.86)" = 1.2,
      "RF 15-gene (AUC=0.85)" = 1.2,
      "FIB-4 (AUC=0.76)" = .7,
      "RF 3-gene published panel (AUC=0.63)" = 0.5
   )) +
   
   scale_color_manual(values = c(
      "RF 57-gene (AUC=0.86)" = "purple4",
      "RF 15-gene (AUC=0.85)" = "mediumpurple",
      "FIB-4 (AUC=0.76)" = "gray60",
      "RF 3-gene published panel (AUC=0.63)" = "black"
   )) +
   ...
   labs(
      x = "1 - Specificity",
      y = "Sensitivity",
      title = "ROC comparison of biomarker panels and FIB-4"
   ) +
...
print(p_roc_all)
ggsave("ROC_Comparison_All_RF_vs_FIB4.pdf", plot = p_roc_all,
...
metrics_all <- metrics_all %>%
   filter(!(Model == "FIB4" & RF_Set != "57-gene"))



# === AUC CI TABLE === #

fib4_ci_low  <- results_list[["57-gene"]]$ci_fib4[1]
fib4_ci_high <- results_list[["57-gene"]]$ci_fib4[3]

auc_ci_df <- data.frame(
   RF_Set = c("57-gene","15-gene","3-gene published panel",
              "57-gene","15-gene","3-gene published panel"),
   Model = c("RF_Model","RF_Model","RF_Model",
             "FIB4","FIB4","FIB4"),
   Metric = "AUC",
   CI_low = c(
      results_list[["57-gene"]]$ci[1],
      results_list[["15-gene"]]$ci[1],
      results_list[["3-gene published panel"]]$ci[1],
      fib4_ci_low,
      fib4_ci_low,
      fib4_ci_low
   ),
   CI_high = c(
      results_list[["57-gene"]]$ci[3],
      results_list[["15-gene"]]$ci[3],
      results_list[["3-gene published panel"]]$ci[3],
      fib4_ci_high,
      fib4_ci_high,
      fib4_ci_high
   )
...
plot_bar_all <- ggplot(metrics_all,
   ...
   geom_text(
      aes(
         label = round(Value,2),
         y = ifelse(Metric == "AUC", Value + 0.1, Value + 0.02)
      ),
      position = position_dodge(width=0.7),
      size=2.1
   ) +
   ...
   labs(title = "Performance comparison of biomarker panels and FIB-4",
        y="Metric Value",
        x="") +
...
ggsave("Metrics_Comparison_All_RF_vs_FIB4.pdf",
       plot = plot_bar_all,
