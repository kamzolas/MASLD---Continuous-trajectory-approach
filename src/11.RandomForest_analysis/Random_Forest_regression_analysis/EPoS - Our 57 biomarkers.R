...
gov_194 <- read_excel("data/Plasma_Proteomics_dataset/Govaeres Top_Proteomics-Transcriptomics.xlsx")
...
gov1= read.csv('data/top_15_BMs_from_ELBOW.csv')[,-1]
...
common_ = intersect(gov, rownames(epos))
epos = epos[ common_, ]
corrected_ucamsanyal = corrected_ucamsanyal[ common_, ]
gov = gov[is.element(gov, common_)]
...
plot(epos_metadata$`predicted trajectory`, 1 - epos_metadata$`real trajectory`, 
...
ggsave("epos_violin_plot_predicted_trajectory_by_disease_group_horizontal.pdf", width = 10, height = 2.2)
...
# Prepare the metadata (keep NAS and Fibrosis as numeric variables)
metadata_df <- as.data.frame(epos_metadata[, c("predicted trajectory",
                                               "Fibrosis.stage", "NAFLD.Activity.Score..NAS..Kleiner")])
colnames(metadata_df) <- c("Predicted_trajectory_position", "Fibrosis", "NAS")
...
ggplot(mydata, aes(x=Trajectory, y=NAS)) +
  geom_violin(trim=FALSE, fill='#A4A4A4', color="#0072B2") +
  geom_boxplot(width=0.1) + theme_minimal()

# Save NAS plot to a PDF
ggsave("epos_violin_predictedtrajectory_NAS.pdf", width = 4.5, height = 2)


# Fibrosis Violin Plot
metadata_df$Fibrosis = as.factor(metadata_df$Fibrosis)
mydata = data.frame("Trajectory" = xaxis, "Fibrosis" = metadata_df$Fibrosis)
ggplot(mydata, aes(x=Trajectory, y=Fibrosis)) +
  geom_violin(trim=FALSE, fill='#A4A4A4', color="#009E73") +
  geom_boxplot(width=0.1) + theme_minimal()

# Save Fibrosis plot to a PDF
ggsave("epos_violin_predictedtrajectory_Fibrosis.pdf", width = 4.5, height = 2)
...
hist(cor(epos_metadata$`predicted trajectory`, 1 - epos_metadata$`real trajectory`), main = "Distribution of R² (PC1 Prediction)", xlab = paste0("R²: ", round(mean(cor(epos_metadata$`predicted trajectory`, 1 - epos_metadata$`real trajectory`))*100, 1), " %"), col = "lightgreen", breaks = 10)
dev.off()

# Print summary statistics
cat("Mean MSE:", mean(mse_list), "\n")
cat("Mean R²:", mean(cor(epos_metadata$`predicted trajectory`, 1 - epos_metadata$`real trajectory`)), "\n")
