...
filename <- paste(main_dir, 'de_and_msviper/de_results.tsv', sep='')
de_results <- read.csv(filename, sep='\t')
gene_scores <- define_gene_scores(de_results)

filename <- paste(main_dir, 'de_and_msviper/msviper_results.tsv', sep='')
...
output_dir <- paste(main_dir, 'network_analysis/initial_weights/', sep='')
