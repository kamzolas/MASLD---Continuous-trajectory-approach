...
# Description
################################################################################
# This scripts defines the weights of all the nodes of MASLD network for the 
# network analysis (propagation). The weights are organised in two matrices (up
# and down weights) and they are different for each sliding window. Their values 
# are derived from the results of differential expression and TF-activity analysis 
# in each sliding window.
# Output files:
# - A new folder (initial_weights) which includes the two matrices in tsv format.
################################################################################


################################################################################
# A function to define weights for genes
################################################################################
...
################################################################################
# A function to define weights for TFs
################################################################################
...
################################################################################
# A function to define the weights for each sw.
################################################################################
...
################################################################################
# 1. Loading of the unified MASLD network and the results from DEA and TF 
# activity analysis.
################################################################################
load(paste(networks_dir, 'MASLD_unified_undirected_network_with_semantics.RData', sep=''))
network <- MASLD_unified_undirected_network

filename <- paste(de_and_tf_analysis_dir, 'de_results.tsv', sep='')
de_results <- read.csv(filename, sep='\t')
gene_scores <- define_gene_scores(de_results)

filename <- paste(de_and_tf_analysis_dir, 'tf_results.tsv', sep='')
msviper_results <- read.csv(filename, sep='\t')
tf_scores <- define_tf_scores(msviper_results)


################################################################################
# 2. Creation of vectors with the personalized weights of network nodes, for 
