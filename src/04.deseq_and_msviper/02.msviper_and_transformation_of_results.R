...
suppressMessages(library(viper)) # 40
suppressMessages(library(purrr)) # 1.2.0

################################################################################
# Description
################################################################################
# Apply msVIPER for the SW-based groups of patients to calcualte the differential
# TF activity. Also the results of DESeq2 are modified and saved in tsv format.
# Outputs:
...
# 1B. Collection of modules gene sets and creation of the unified gene set
modules_df <- read.table(modules_filename, sep='\t', header=TRUE)
...
################################################################################
# 3. Loading of CollectTRI database and creation of the 'background_genes' variable
