...
# used in next python script (22) to find the annotated gene symbols and their 
...
modules_dir = paste('../../results/wgcna_and_linear_modelling/grid_params/', key, '/', sep='')
output_dir = paste('../../results/networks/', sep='')
tmp_output_dir = paste('../../results/networks/MASLD_network_construction/', sep='')
...
################################################################################
#
# 3. Similarly as above, a gene set related to the differentially activated TFs 
...
filename <- paste(output_dir, 'de_and_msviper/msviper_results.tsv', sep='')
...
################################################################################
#
# 4. In this step two lists are created to store the TFs and genes in related 
...
reactome_terms_df <- read.csv2('../../data/annotation_databases/reactome_graph.tsv', 
                               sep='\t', header=TRUE)
signalling_terms_df <- read.csv2('../../data/annotation_databases/reactome_signalling_terms.tsv', 
                                 sep='\t', header=TRUE)
...
################################################################################
#
# 5. Similarly to the previous step, a vector of genes, related to the 
...
################################################################################
#
# 6. Loading and transformation of reference PPI network into the appropriate 
...
#
################################################################################
load(paste(output_dir, 'reference_network.RData', sep=''))
...
################################################################################
#
# 7. Creation of an upstream network for each MASLD variable, based on 
...
upstream_networks_per_var <- sapply(names(upstream_gene_sets_per_var), function(var) {
  ...
  seeds_scores <- rep(100, length(seeds_vector))
...
################################################################################
#
# 8. Creation of a downstream network for each MASLD variable, based on 
# the reference one and the PCSF algorithm. Before the construction of these 
...
################################################################################
#
# 9. This final step is used to gather together all the genes in upstream and
