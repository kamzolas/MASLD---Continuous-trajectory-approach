...
#deep_split = 4
#min_size = 180
edge_weight_attr = 'lapl_norm_weight'
randomization_method = 'sw'
...
################################################################################
#
# 2. Loading of the network and initial weights (for DEA and TF activity 
# analysis).
#
################################################################################
...
filename <- paste(key_dir, 'network_analysis/initial_weights/up.tsv', sep='')
up_scores <- read.csv(filename, sep='\t')

filename <- paste(key_dir, 'network_analysis/initial_weights/down.tsv', sep='')
...
################################################################################
#
# 3. Network analysis for each sw.
#
################################################################################
sw <- 'SW2'
...
for (sw in SWs) {
    ...
    # Separate the scores in up and down
...
output_dir = paste(key_dir, 'network_analysis/propagation_results/', sep='')
